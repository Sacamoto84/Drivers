...
ee_u16 core_bench_list(core_results *res, ee_s16 finder_idx) {  
    ...
    for (i=0; i<find_num; i++) {  
        ...
        if (info.idx>=0)  
            info.idx++;  
#if CORE_DEBUG  
    SEGGER_RTT_printf(0,"List find %d: [%d,%d,%d]\n",i,retval,missed,found);  
#endif  
    }  
...
#if CORE_DEBUG  
    SEGGER_RTT_printf(0,"List sort 1: %04x\n",retval);  
#endif  
...
#if CORE_DEBUG  
    SEGGER_RTT_printf(0,"List sort 2: %04x\n",retval);  
#endif  
    return retval;  
}  
...
list_head *core_list_init(ee_u32 blksize, list_head *memblock, ee_s16 seed) {  
    ...
    list = core_list_mergesort(list,cmp_idx,NULL);  
#if CORE_DEBUG  
    SEGGER_RTT_printf(0,"Initialized list:\n");  
    finder=list;  
    while (finder) {  
        SEGGER_RTT_printf(0,"[%04x,%04x]",finder->info->idx,(ee_u16)finder->info->data16);  
        finder=finder->next;  
    }  
    SEGGER_RTT_printf(0,"\n");  
#endif  
