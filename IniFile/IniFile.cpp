...
bool IniFile::validate(char* buffer, size_t len) const
{
	uint32_t pos = 0;
	error_t err;
	while ((err = readLine(_fatfsfile, buffer, len, pos)) == errorNoError)
		;
...
bool IniFile::browseSections(char* buffer, size_t len, IniFileState &state) const
{
	...
	do {
		err = IniFile::readLine(_fatfsfile, buffer, len, state.readLinePosition);
...
IniFile::error_t IniFile::readLine(FIL &file, char *buffer, size_t len, uint32_t &pos)
{
	//if (!_file)
	...
	UINT bytesRead;
	f_read(&file, buffer, len, &bytesRead);
						  ...
						  IniFileState &state) const
{
	...
	error_t err = IniFile::readLine(_fatfsfile, buffer, len, state.readLinePosition);
...
void IniFile::printErrorMessage(bool eol)
{
	SEGGER_RTT_WriteString(0, "IniFile::printErrorMessage>");
	switch (_error) {
	  case errorNoError:
		  SEGGER_RTT_WriteString(0, "no error");
	    break;
	  case errorFileNotFound:
		  SEGGER_RTT_WriteString(0, "file not found");
	    break;
	  case errorFileNotOpen:
		  SEGGER_RTT_WriteString(0, "file not open");
	    break;
	  case errorBufferTooSmall:
		  SEGGER_RTT_WriteString(0, "buffer too small");
	    break;
	  case errorSeekError:
		  SEGGER_RTT_WriteString(0, "seek error");
	    break;
	  case errorSectionNotFound:
		  SEGGER_RTT_WriteString(0, "section not found");
	    break;
	  case errorKeyNotFound:
		  SEGGER_RTT_WriteString(0, "key not found");
	    break;
	  case errorEndOfFile:
		  SEGGER_RTT_WriteString(0, "end of file");
	    break;
	  case errorUnknownError:
		  SEGGER_RTT_WriteString(0, "unknown error");
	    break;
	  default:
		  SEGGER_RTT_WriteString(0, "unknown error value");
	    break;
	  }
