...
#define max(a,b) ((a)>(b)?(a):(b))	// привет esp
...
long map(long x, long in_min, long in_max, long out_min, long out_max){  return (x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;}
  ...
  GStepper(int stepsPerRev, GPIO_TypeDef * gpin1 , uint32_t pin1, GPIO_TypeDef *gpin2, uint32_t pin2 , GPIO_TypeDef *gpin3 , uint32_t pin3 = -1 , GPIO_TypeDef * gpin4 = NULL , uint32_t pin4 = -1, GPIO_TypeDef * gpin5 = NULL,  uint32_t pin5 = -1) : 
	_stepsPerDeg(stepsPerRev / 360.0) {
		if (_DRV == STEPPER2WIRE) {
			configurePin(0, gpin1, pin1);
			configurePin(1, gpin2, pin2);
			...
			configurePin(0, gpin1, pin1);
			configurePin(1, gpin2, pin2);
			configurePin(2, gpin3, pin3);
			configurePin(3, gpin4, pin4);
	...
	void disable() {
		...
		{
			if (_enDir){
		    GPIO_WritePin( _GPIO_en , _enPin , 0);}
			else{
				GPIO_WritePin( _GPIO_en , _enPin , 1);}
		}
	}	
...
private:
	...
	void configurePin(int num, GPIO_TypeDef *gpin, uint32_t pin) {
		_pins[num].GPIO =   gpin;
		_pins[num].Pin  =   pin;
	}

	
	void setPin(int num, uint8_t state) 
  { 
		GPIO_WritePin( _pins[num].GPIO, _pins[num].Pin, state);	
	}
	...
	GPIO_TypeDef  *       _GPIO_en; 
	uint16_t              _enPin = -1;
	
	GPIO_TypeDef  *       _GPIO_dir; 
	uint16_t              _dirPin = -1;
