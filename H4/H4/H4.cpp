...
void HAL_enableInterrupts() {  __enable_irq (); } // разрешить прерывания
void HAL_disableInterrupts(){ __disable_irq (); } // запретить прерывания
...
void h4reboot(){ NVIC_SystemReset(); }
...
void task::schedule(){ at= uwTick + randomRange(rmin,rmax); }
...
void H4::loop(){
	task* t=nullptr;
    uint32_t now = uwTick; // can't do inside loop...clocks dont work when HAL_disableInterrupts()!!!
	HAL_disableInterrupts();
	if(size()){
        if(((int)(top()->at -  now)) < 1) {
            t=top();
