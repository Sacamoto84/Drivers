...
int16_t ADS1X15::_readADC(uint16_t readmode)
{
  ...
  if (_mode == ADS1X15_MODE_SINGLE)
  {
    while ( isBusy() )
        ...
        HAL_Delay(_conversionDelay);      // TODO needed in continuous mode?
  }
  else
  {
    HAL_Delay(_conversionDelay);      // TODO needed in continuous mode?
...
void ADS1X15::_writeRegister(uint8_t address, uint8_t reg, uint16_t value)
{
  uint8_t dt[3];
  dt[0] = (value >> 8);
  dt[1] = (value & 0xFF);

  HAL_I2C_Mem_Write(&hi2c2, address << 1, reg, 1, &dt[0], 2, 1000);
}

uint16_t ADS1X15::_readRegister(uint8_t address, uint8_t reg)
{
   uint8_t dt[2];
   HAL_I2C_Mem_Read(&hi2c2, address << 1, reg, 1, &dt[0], 2, 1000);
