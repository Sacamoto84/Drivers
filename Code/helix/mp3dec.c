...
// +++ I.I. 15.05.2011
typedef void (*LoopCallBackProc)(void);
LoopCallBackProc MP3DecoderCallBack = 0;     
...
static int MP3FindFreeSync(unsigned char *buf, unsigned char firstFH[4], int nBytes)
{
	 ...
	 *  - run out of nBytes (MP3FindSyncWord() returns -1)
...
{
	static int offset, bitOffset, mainBits, gr, ch, fhBytes, siBytes, freeFrameBytes;
	static int prevBitOffset, sfBlockBits, huffBlockBits;
	static unsigned char *mainPtr;
	MP3DecInfo *mp3DecInfo = (MP3DecInfo *)hMP3Decoder;
	...
	*bytesLeft -= (fhBytes + siBytes);
	...
	for (gr = 0; gr < mp3DecInfo->nGrans; gr++) {
        ...
        if (MP3DecoderCallBack) MP3DecoderCallBack();
	}
	return ERR_MP3_NONE;
}
