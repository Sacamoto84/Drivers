		...
		while (pY < Y + bmp->y) 
		{
			...
			while (pX < X + bmp->x) {
				...
				if (tmpCh) 
				{
					while (bL < 8) 
					{
						if (tmpCh & 0x01) uTFT_SetPixel(LCD, pX,pY + bL,1);
						tmpCh >>= 1;
						if (tmpCh) 	bL++;
						else { pX++;	break; }
					}
...
void uTFT_Bitmap_From_Flash_Alpha(uTFT_LCD_t* LCD, int32_t x0, int32_t y0, Bitmap * bmp, float customAlpha)
{
	...
	volatile uint8_t  dR,dG,dB;
		...
		for(index = 0; index < index_max; index++)
		{
			...
			x = (index % (bmp->x)) + x0;
			y = bmp->y - (index / (bmp->x)) - 1 + y0;	

			  sAlpha_Float = (float)((bmp->Palette[index] >> 24) / 255.0F) * customAlpha;
				
			  sR = bmp->Palette[index] >> 16;
				sG = bmp->Palette[index] >> 8;
				sB = bmp->Palette[index] & 0xFF;
				
				dColor = uTFT_GetPixel(LCD, x, y);
...
volatile uint8_t bmp_header_buffer[54];        //Буффер заголовка
volatile uint8_t buf[5100]; //Буффер данных
UINT bytesread;
...
void uTFT_BMP_From_File(uTFT_LCD_t* LCD,int32_t x0, int32_t y0, char * Name)
{
	...
	if (res == FR_OK)
	{
		...
		f_read (&USERFile, &bmp_header_buffer, 54, bytesread);
		...
		{	
		  ...
		  f_read (&USERFile, &bmp_buffer_color_table, *bmp_header.biClrUsed*4, bytesread);
		  ...
		  {
			  if (index % 4096 == 0)
					f_read (&USERFile, &buf, 4096, bytesread);
				
			  x = (index % (*bmp_header.biWidth)) + x0;
			  y = *bmp_header.biHeight - (index / (*bmp_header.biWidth)) - 1 + y0;	
				uTFT_SetPixel(LCD ,x,y, bmp_color_table[buf[index % 4096]]);	
		  }
		...
		else //Палитра не используется
		{
			if (*bmp_header.biBitCount == 24)
			{
				...
				{
					if (index % 1365 == 0) 
						f_read (&USERFile, &buf, 4095, bytesread);
					
					x = (index % *bmp_header.biWidth) + x0;
					y = *bmp_header.biHeight - (index / *bmp_header.biWidth) - 1 + y0;	
					uTFT_SetPixel(LCD, x,y, RGB565(buf[(index % 1365)*3+2], buf[(index % 1365)*3+1],buf[(index % 1365)*3]));
				}
			}
...
void uTFT_BMP_From_File_Alpha(uTFT_LCD_t* LCD, int32_t x0, int32_t y0, char * Name, int offset)
{
	...
	if (res == FR_OK)
	{
		...
		if (offset) f_read (&USERFile, &buf, offset, bytesread);
